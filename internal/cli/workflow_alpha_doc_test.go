...
func TestWorkflowAlphaTranscriptCoversRequiredCommandSequence(t *testing.T) {
	...
	requiredSnippets := []string{
		...
		"| `ph menu` | inside tmux | open the popup in the current tmux client |",
		"| `ph menu` | outside tmux from the default `ssh agent` entry path | join the shared `default` tmux session first, then open the popup there |",
		"| `ph menu` | outside tmux from any other shell in the VM | use the same bootstrap as the default `ssh agent` path |",
		"| `ph attach <workstream-id>` | inside tmux | switch the current client directly into `ph:<workstream-id>` |",
		"| `ph attach <workstream-id>` | outside tmux from the default `ssh agent` entry path | join tmux and attach directly to `ph:<workstream-id>` |",
		"| `ph attach <workstream-id>` | outside tmux from any other shell in the VM | use the same bootstrap as the default `ssh agent` path |",
