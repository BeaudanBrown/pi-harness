...
export const CHECKPOINT_ENTRY_TYPE = "remote-session.checkpoint";
...
export interface RemoteCheckpoint {
	...
	checkpointId: string;
	createdAt: string;
	input: RemoteCheckpointInput;
	body: string;
}

export interface StoredCheckpointBoundary extends RemoteCheckpoint {
	bindingId: string;
	status: "prepared" | "waiting";
...
export function renderRemoteCheckpoint(input: RemoteCheckpointInput): string {
	...
	if (body.length > MAX_CHECKPOINT_BODY_LENGTH) throw new Error("Rendered checkpoint exceeds the Matrix message limit");
	return body;
}

export function createRemoteCheckpoint(value: unknown): RemoteCheckpoint {
	const input = validateRemoteCheckpoint(value);
	return {
		version: 1,
		checkpointId: randomUUID(),
		createdAt: new Date().toISOString(),
		input,
		body: renderRemoteCheckpoint(input),
	};
}

function parseStoredCheckpoint(value: unknown, bindingId: string): StoredCheckpointBoundary | undefined {
	if (typeof value !== "object" || value === null) return undefined;
	const candidate = value as Partial<StoredCheckpointBoundary>;
	if (
		candidate.version !== 1 ||
		candidate.bindingId !== bindingId ||
		(candidate.status !== "prepared" && candidate.status !== "waiting") ||
		typeof candidate.checkpointId !== "string" ||
		typeof candidate.createdAt !== "string" ||
		typeof candidate.transactionId !== "string" ||
		typeof candidate.body !== "string" ||
		candidate.body.length > MAX_CHECKPOINT_BODY_LENGTH ||
		!Array.isArray(candidate.inboundEventIds) ||
		!candidate.inboundEventIds.every((eventId) => typeof eventId === "string")
	) {
		return undefined;
	}
	try {
		candidate.input = validateRemoteCheckpoint(candidate.input);
		if (candidate.body !== renderRemoteCheckpoint(candidate.input)) return undefined;
		if (candidate.transactionId !== `remote-checkpoint-${candidate.checkpointId}`) return undefined;
	} catch {
		return undefined;
	}
	return candidate as StoredCheckpointBoundary;
}

export function restoreCheckpointBoundaries(
	entries: readonly unknown[],
	bindingId: string,
): StoredCheckpointBoundary[] {
	const checkpoints = new Map<string, StoredCheckpointBoundary>();
	for (const entry of entries) {
		if (typeof entry !== "object" || entry === null) continue;
		const candidate = entry as { type?: unknown; customType?: unknown; data?: unknown };
		if (candidate.type !== "custom" || candidate.customType !== CHECKPOINT_ENTRY_TYPE) continue;
		const checkpoint = parseStoredCheckpoint(candidate.data, bindingId);
		if (checkpoint) checkpoints.set(checkpoint.checkpointId, checkpoint);
	}
