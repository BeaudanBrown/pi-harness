...
import { fileRefs, type ChatReply } from "./file-types.js";

export const MODEL_TIMEOUT_MS = 90000;
export const WORKSPACE_TIMEOUT_MS = 600000;
export function createQuestionServer(execute: (question: string, signal: AbortSignal, workspace: boolean) => Promise<ChatReply>, timeoutMs = MODEL_TIMEOUT_MS): http.Server {
	let busy = false;
	const server = http.createServer(async (req, res) => {
		...
		try {
			...
			const files = typeof result === "string" ? [] : fileRefs(result.files);
			if (typeof answer !== "string" || !answer || Buffer.byteLength(answer) > MAX_ANSWER) throw Error("Answer size");
			if (!res.destroyed && !res.writableEnded) { res.writeHead(200, { "content-type": "application/json", connection: "close" }); res.end(JSON.stringify({ answer, ...(files.length ? { files } : {}) })); }
...
export function modelAnswer(socketPath: string, question: string, signal?: AbortSignal, workspace = false, filesEnabled = false): Promise<ChatReply> {
	const timeout = (workspace || filesEnabled ? WORKSPACE_TIMEOUT_MS : MODEL_TIMEOUT_MS) + 5000;
	return new Promise((resolve, reject) => {
		const request = http.request({ socketPath, path: "/answer", method: "POST", signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(timeout)]) : AbortSignal.timeout(timeout),
			...
			response.on("data", chunk => { size += chunk.length; if (size > MAX_ANSWER * 6 + 16384) { response.destroy(Error("Response size")); return; } chunks.push(Buffer.from(chunk)); });
			response.on("end", () => {
				try {
					const value = JSON.parse(Buffer.concat(chunks).toString("utf8"));
					if (response.statusCode !== 200 || !value || Object.keys(value).some(key => !["answer", "files"].includes(key)) || typeof value.answer !== "string" || !value.answer || Buffer.byteLength(value.answer) > MAX_ANSWER) throw Error("Worker unavailable");
					const files = value.files === undefined ? [] : fileRefs(value.files);
					resolve(files.length ? { text: value.answer, files } : value.answer);
