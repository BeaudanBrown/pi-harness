...
in {
  options.services.pi-harness.bridgeChat.assistant = {
    ...
    projectCommands = lib.mkOption {
      ...
      description = "Descriptions of host-approved named commands; execution argv belongs to workspaceExecutor.commands, not the model or project files.";
    };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      { assertion = builtins.length cfg.workspaceRoomIds <= 256 && privatePath cfg.workspaceSocket && lib.all (room: cfg.allJoinedRooms || builtins.elem room cfg.roomIds) cfg.workspaceRoomIds; message = "Workspace rooms must be enabled chat rooms and the executor socket must be an absolute runtime path."; }
