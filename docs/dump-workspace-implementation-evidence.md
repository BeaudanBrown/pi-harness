...
| #98 | Path, symlink, hardlink, special-file safety | Descriptor-rooted operations; adversarial file tests | passed within declared single-writer threat model |
...
- Spec medium (final pathname compare/commit race with external writers): added project-wide lifetime lease, separate-state executor exclusion and admitted-writer lock tests. Clarified that digest checks are NOT atomic CAS against uncooperative host writers. #101 owns enforcing exclusive deployment access; existing GRILL writable export/other sessions must be addressed before activation. The advisory lease alone is not claimed to solve arbitrary external writes. Operator decision on restricting GRILL editing is pending.
