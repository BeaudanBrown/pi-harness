...
The supervisor closes a child only after its matching handoff is durable and independent review confirms all acceptance criteria. It closes an epic only when every descendant is closed with durable supervised child evidence, project verification passes, and epic-level acceptance evidence is complete; no separate mandatory epic review is required.
