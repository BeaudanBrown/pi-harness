...
Media IPC never carries a filesystem path. Inbound images and explicit outbound artifacts use blob/upload identities and bounded begin/chunk operations whose encoded NDJSON frame remains at most 64 KiB. The private host spool permits at most 128 blobs, 25 MiB per blob, and 256 MiB total. A missing HTTP media Content-Length is not a zero-byte declaration; a present header must be well-formed and agree, and streamed length is always enforced. Outbound image decoding is bounded and occurs before IPC, without changing the bytes described by the upload digest. Linux artifact traversal pins directory descriptors and rejects links at every requested segment. Durable artifact phases may carry a redacted terminal media failure; these cease to reference spool bytes. Live artifact retries are bounded, cancellation-aware and isolated from relay startup and other exports. A blob is committed only after declared length and SHA-256 verification; incomplete, expired, rejected, and consumed blobs are removed, with cleanup idempotent across restart. Workspace artifact handles are resolved and canonicalized by the host; adapters cannot nominate paths.
