...
Coordinator refresh is distinct from stop and context reset. A confirmed project refresh preserves its room, active generation/session, preferences, checkpoint and queued input. Delivery is held while a live authenticated adapter checks idle state and requests graceful shutdown without abort. The relay refuses busy/unsettled work and unknown live activity, waits for disconnection, then clears only the exact managed window and relaunches the same session with complete host-resolved placement. The attachment acknowledgment carries the manifest's portable placement, avoiding dependence on an external launcher's environment allowlist; canonical filesystem paths still do not cross IPC. No persisted task or cancellation marker is introduced. If interrupted, normal registry reconciliation preserves the session/queue and an explicit repeat refresh or resume recovers the launch. It neither upgrades the relay nor modifies the guaranteed coordinator.
...
A bootstrap `/sync` establishes its validated cursor without interpreting retained timeline events, including limited timelines. An established-cursor limited timeline instead requires bounded forward `/messages` recovery from the saved sync token to the fixed new sync boundary before any batch input is dispatched. Recovery preserves event order and identity, verifies coverage of the sync tail and explicit start/end pagination boundaries (never inferring completeness from a missing end), and admits at most 512 events per recovered room, ten pages of at most 100 events, and the existing 4 MiB response bound. Failure retains the old cursor; interrupted acceptance retries stable delivery identities. Larger or inaccessible histories require operator recovery rather than a silent skip. Recovery progress within the bounded interval is intentionally retried from the saved cursor, not stored as a second cursor authority.

### Durable intake and notice delivery amendment (#94)

After gap recovery, candidate events are persisted in a bounded host-local inbox
before the registry cursor advances. Current membership is checked by independent,
ordered room workers before acceptance for Pi. The inbox retains the last batch
identity to deduplicate the write-before-cursor crash window. Recovery of an
uncertain poll is scoped to its room, not a prerequisite for host-wide intake.
Relay notices have a durable outbox that freezes payload/session identity before
sending; lifecycle success depends on accepting the notice locally, not displaying
it remotely. Conflicting historical projections remain blocked records, isolated
from later work. Both stores belong to the persistent host backup boundary; an
older relay that ignores them is not a supported downgrade. Existing adapter wire
messages remain supported. See [reliability operations](../../managed-matrix-reliability.md)
for bounds, evidence, rate-limit behavior and deployment acceptance.
...
The epic uses one explicit temporary `1.x` compatibility strategy while vertical feature slices land before the final cutover owned by #63. The relay, adapter, and host state are distributed as one atomic package and may use strict optional projections of reviewed V2 fields under the existing envelope during this staging period; arbitrary mixed package versions and downgrade are unsupported. The restart-safe update amendment below defines the one explicitly supported rolling baseline. Generation IDs already use their final V2 derivation domains. Compatibility manifests retain both active convenience fields and the complete append-only generation history, and the migration validates the synchronized manifest/runtime bundle before preserving that history and metadata in the V2-only shape. Successful exact model and thinking selections are separately persisted as current conversation preferences only after the relay validates the typed result against its pending authorized control; restarts and fresh generations restore both preferences without rewriting prior generations. The host launcher validates and forwards those optional selections to project and coordinator tmux environments. A typed status result supplies only live idle/busy, actual model/thinking, and balanced context usage; the relay composes it with authoritative bounded manifest/runtime fields and projects a redacted status notice, making requested/runtime model mismatches explicit. The compatibility reader does not accept a `2.0.0` store, and the migration output is not activated as the live store until #63 switches all runtime readers and writers together. No later slice may reinterpret or flatten compatibility generation history.

V2 controls are typed operations, never prompt text. Every accepted joined participant has equal control and session-management authority. State-changing controls are accepted only while idle. Typing represents live agent activity, never control/slash-command dispatch or attachment alone. Finalization reception and disconnect clear that live signal independently of activity-card delivery; durable input and activity records remain authoritative for recovery. Typing sends are per-conversation single-flight, coalesce current intent, and carry bounded deadlines with shutdown cancellation; Matrix outages may still expire server-side feedback. Attachment loss finalizes an interrupted card after grace. A typed `activity_interrupted` error lets a surviving adapter project a deterministic continuation identity derived from the prior ID, preserving run measurements without mutating finalized history or injecting input; reconnect replays live activity even without a new tool event. Other terminal outcomes remain immutable errors. Control-selection and option-bearing checkpoint polls request a room notification through deterministic `m.mentions` content; participant push settings remain authoritative and poll closures do not notify. A checkpoint's first valid currently joined participant vote or ordinary-text bypass resolves it while later resolutions are invalid. Tool activity may expose only a bounded tool name and state. Arguments, paths, commands, output, partial answers, and reasoning have no contract field.

Media IPC never carries a filesystem path. Inbound images and explicit outbound artifacts use blob/upload identities and bounded begin/chunk operations whose encoded NDJSON frame remains at most 64 KiB. The private host spool permits at most 128 blob records. Outbound artifacts retain 25 MiB per-file and 256 MiB aggregate byte quotas; a durable optional inbound marker exempts incoming images from byte quotas on commit and recovery. New inbound blob IDs use a distinct length-framed inbound-blob derivation domain, so inbound and outbound metadata/quota policies cannot collide for identical content; old recorded IDs remain readable for recovery. External Matrix image content permits additive metadata rather than applying IPC-style exact-key validation. Only URL and bounded plain-caption fields are consumed; optional info hints are ignored without coercion and the downloaded signature selects the supported MIME type. Known encrypted-file, relation/thread and edit semantics remain explicitly excluded. Captionless or filename-only bodies use a neutral prompt. Incoming images use original downloaded bytes without harness size/pixel admission checks, metadata equality checks, decoding or normalization; HTTP framing belongs to fetch, and Matrix/model/machine-resource limits still apply. Legacy required dimension fields are neutral placeholders, not model input. Internal actual-byte counts, chunk counts and digests still protect transfer/recovery integrity. Older unmarked spool records remain readable with their previous bounds; mixed-version downgrade is unsupported. Outbound image decoding is bounded and occurs before IPC, without changing the bytes described by the upload digest. Linux artifact traversal pins directory descriptors and rejects links at every requested segment. Durable artifact phases may carry a redacted terminal media failure; these cease to reference spool bytes. Live artifact retries are bounded, cancellation-aware and isolated from relay startup and other exports. A blob is committed only after internal actual-length and SHA-256 verification; incomplete, expired, rejected, and consumed blobs are removed, with cleanup idempotent across restart. Workspace artifact handles are resolved and canonicalized by the host; adapters cannot nominate paths.
...
## Restart-safe update amendment (#93)

Tmux server, Pi process and relay have independent lifetimes. The consuming host
owns one shared foreground tmux server in its own user unit. Relay replacement
must not stop it or any existing Pi process. A host-owned rollout guard rejects
legacy server ownership before stopping the old relay; no unsafe adoption or
automatic maintenance restart is permitted. The harness uses a guarded,
single-transaction relay restart instead of NixOS's split stop/start sequence.
Existing adapters reconnect using the unchanged nonce and receipt protocol.

The `ae957fa` adapter wire subset is the initial supported rolling baseline for
this update. State schema, wire contract and executable revision are distinct;
unknown versions remain errors. Later incompatible updates require an explicit
maintenance/migration decision, never automatic busy-session refresh. See
