...
- Keep chat intake, model calls, isolated project operations and publication on **NAS**. GRILL is not required for this workflow.
...
The current submit command creates a fresh request ID each time; add an idempotent prepare/submit/status seam. Persist operation identity before dispatch and reconcile the same identity after restart/timeout. Never blindly repeat edits or publication after an uncertain result. Serialize project writes using the executor's exclusive project lease. Before activation, stop/exclude uncooperative GRILL/human editors; a final digest check is not atomic compare-and-swap with arbitrary external processes. Any admitted writer must honour the same lease.
